...
function Box({color}) {
    ...
    function getInitialPosition(){
        let v = new Vector3( (Math.random() * 2 - 1) * 3 ,Math.random() *2.5 +0.1 ,(Math.random()*2 -1 ) * 20);
        if (v.x < 0) v.x -= 3;
        if (v.x > 0) v.x += 3;
...
export function Boxes() {
    ...
    {
        arr.map(
            (e,i) => <Box key={i} color= {i % 2 === 0 ? [0.4, 0.1, 0.1] : [0.05, 0.15, 0.4] } />
        )
    }
    </>
}